...
function RoomPreview(props) {
	...
	return (
		<>
				...
				{!props?.room && !props?.error ? (
					...
					<>
						...
						<section className="single-room">
							...
							{/* room reviews list */}
							<div className="single-room-images">
								{props?.room?.data?.id && <RoomReviewList roomId={props?.room?.data?.id} />}
							</div>
						</section>
					</>
				)}
			...
			{bookingModal.open && (
				<OrderPlaceModal bookingModal={bookingModal} setBookingModal={setBookingModal} />
			)}
		</>
	)
}
...
export async function getServerSideProps(ctx) {
	try {
		...
		return {
			props: {
				room: null,
				error: JSON.parse(JSON.stringify(err?.data)),
			},
		}
	}
}
